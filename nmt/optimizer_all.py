...
class AdaShift(optimizer.Optimizer):
    ...
    def __init__(self, learning_rate=0.001, keep_num=10, use_mov=False, mov_num=10, beta1=0.9, beta2=0.999, epsilon=1e-10, pred_g_op='none', use_locking=False, name="AdaShift"):
        super(AdaShift, self).__init__(use_locking, name)
        self._lr = learning_rate
        self._keep_num = keep_num
        if not use_mov:
            self._mov_num = keep_num
        else:
            self._mov_num = mov_num
        self._beta2 = beta2
        self._beta1 = beta1
        self._epsilon = epsilon
        self._pred_g_op = pred_g_op

        s = np.asarray([(self._beta1**(self._mov_num-i-1)) for i in range(self._mov_num)])
        self.s = s / np.sum(s) 
...
class AdamSpace(optimizer.Optimizer):

    def __init__(self, learning_rate=0.001, beta1=0.9, beta2=0.999, epsilon=1e-10, use_locking=False, name="AdamSpace"):
        super(AdamSpace, self).__init__(use_locking, name)
        self._lr = learning_rate
        self._beta1 = beta1
        self._beta2 = beta2
        self._epsilon = epsilon
    ...
    def _create_slots(self, var_list):
        ...
        for v in var_list:
            self._zeros_slot(v, "m", self._name)
            self._zeros_slot(v, "v", self._name)
            self._zeros_slot(v, "gg", self._name)
    ...
    def _apply_dense(self, grad, var):
        ...
        m_t = state_ops.assign(m, beta1_t * m + grad * (1 - beta1_t), use_locking=self._use_locking)
        # v_t = state_ops.assign(v, beta2_t * v + (grad * grad) * (1 - beta2_t), use_locking=self._use_locking)
        v_t = state_ops.assign(v, v * beta2_t + tf.reduce_max(tf.square(grad)) * (1 - beta2_t), use_locking=self._use_locking)
