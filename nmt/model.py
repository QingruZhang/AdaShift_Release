  ...
  def _set_train_or_infer(self, res, reverse_target_vocab_table, hparams):
    ...
    if self.mode == tf.contrib.learn.ModeKeys.TRAIN:
      ...
      if hparams.optimizer == "sgd":
          ...
          opt = NadamOptimizer(learning_rate=self.learning_rate, beta1=hparams.beta1, beta2=hparams.beta2, epsilon=hparams.epsilon)
      elif hparams.optimizer == "adamspace":
          opt = optimizer_all.AdamSpace(learning_rate=self.learning_rate, beta1=hparams.beta1, beta2=hparams.beta2, epsilon=hparams.epsilon)
      else:
          # assert 'No optimizer has been chosed, name may be wrong'
          opt = tf.train.MomentumOptimizer(learning_rate=self.learning_rate, momentum=0.9, use_nesterov=False)
