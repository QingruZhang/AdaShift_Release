...
class DenseNet:
    def __init__(self, data_provider, growth_rate, depth,
                 ...
                 bc_mode=False,
                 exp_name="",
                 optimizer="", beta1=0.9, beta2=0.999, epsilon=1e-8, keep_num=10, pred_g_op="max",
                 use_mov=0, mov_num=10,
                 **kwargs):
        """
        ...
        self.pred_g_op = pred_g_op
        self.use_mov = (use_mov==1)
    ...
    def _build_graph(self):
        ...
        if self.optimizer == "adam":
            ...
            optimizer = optimizer_all.Adam(learning_rate=self.learning_rate, beta1=self.beta1, beta2=self.beta2, epsilon=self.epsilon)
        elif self.optimizer == "adaShift":
            print("adaShift optimizer: %.4f, %.4f, %d" % (self.beta1, self.beta2, self.keep_num))
            optimizer = optimizer_all.AdaShift(learning_rate=self.learning_rate, beta1=self.beta1, beta2=self.beta2,
                                                epsilon=self.epsilon, keep_num=self.keep_num, pred_g_op=self.pred_g_op,
                                                use_mov=self.use_mov, mov_num=self.mov_num)
        elif self.optimizer == "amsgrad":
            print("amsgrad optimizer: %.4f, %.4f" % (self.beta1, self.beta2))
            optimizer = optimizer_all.AMSGrad(learning_rate=self.learning_rate, beta1=self.beta1, beta2=self.beta2,
                                             epsilon=self.epsilon)
        elif self.optimizer == "adamspace":
            print("adamspace optimizer: %.4f, %.4f" % (self.beta1, self.beta2))
            optimizer = optimizer_all.AdamSpace(learning_rate=self.learning_rate, beta1=self.beta1, beta2=self.beta2, epsilon=self.epsilon)
        elif self.optimizer == "nadam":
            print("nadam optimizer: %.4f, %.4f" % (self.beta1, self.beta2))
            optimizer = NadamOptimizer(learning_rate=self.learning_rate, beta1=self.beta1, beta2=self.beta2, epsilon=self.epsilon)
        else:
